...
<body>
  ...
  <article>
    ...
    <div class="picsbox">
      <div class="banner">
        <div id="banner" class="fader">
          @foreach ($lunb as $v)
          <li class="slide"><a href="/info/{{ $v['id'] }}" target="_blank"><img src="{{ $v['img'] }}"><span class="imginfo">{{ $v['title'] }}</span></a></li>
          @endforeach
      ...
      <!--banner end-->
      <div class="toppic">
        <li> <a href="/info/{{ $ro['id'] }}" target="_blank"> <i><img src="{{ $ro['img'] }}"></i>
            <h2>{{ $ro['title'] }}!</h2>
            <span>{{ $ro['cat'] }}</span>
          </a> </li>
        <li> <a href="/info/{{ $rt['id'] }}" target="_blank"> <i><img src="{{ $rt['img'] }}"></i>
            <h2>{{ $rt['title'] }}</h2>
            <span>{{ $rt['cat'] }}</span>
          </a> </li>
      </div>
    ...
    <div class="blank"></div>
    <!--blogsbox begin-->
    <div id='blog' class="blogsbox">
      @foreach ($data as $v)
        <div class="blogs" data-scroll-reveal="enter bottom over 1s">
          <h3 class="blogtitle"><a href="/info/{{ $v['id'] }}" target="_blank">{{ $v['title'] }}</a></h3>
          @if (count($img[$v['id']])==1)
            <span class="blogpic"><a href="/info/{{ $v['id'] }}" title="">@foreach ($img[$v['id']] as $i)<img src="{{ $i->src }}" alt="">@endforeach</a></span>
          @elseif(count($img[$v['id']])==3)
            <span class="bplist">
              <a href="/info/{{ $v['id'] }}" title="">
                @foreach ($img[$v['id']] as $i)
          ...
          <div class="bloginfo">
            <ul>
              <li class="lmname"><a href="/list/{{ $v['cid'] }}">{{ $v['cat'] }}</a></li>
